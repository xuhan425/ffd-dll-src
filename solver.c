...
int FFD_solver(PARA_DATA *para, REAL **var, int **BINDEX) {
  ...
  double t_cosim;
  int flag, next, bar=0;
  int output_t = 786; /*time step to begin outputting dynamic file*/
  char tmpName[10];
  ...
  ***************************************************************************/
  next = 1;
  while(next==1) {
	  /*Outputs dynamic file*/
	  /*if (para->outp->OutputDynamicFile == 1)
		  if (fabs(para->mytime->t - output_t) < SMALL && para->mytime->t <= 786 && para->mytime->t >= 786) {
